 ...
 *
 * <p>To obtain the (mutually exclusive) token parsers, use the {@link #tokenParser(Parser)}
 ...
 *
 * <p>This class maintains a {@link Memoizer} (as a parse state: {@link #memoState}) to map input
...
public final class Tokens
{
     ...
     */
    public final ParseState<Memoizer> memoState;
    ...
    public Tokens (Supplier<Memoizer> memo) {
        this.memoState = new ParseState<>(Tokens.class, memo);
    }
     ...
     * {@link Parser#excludeErrors} flag will be set to true.
     */
    public TokenParser tokenParser (Parser baseParser)
    {
        if (NArrays.contains(parsers, baseParser))
            return new TokenParser(this, baseParser);
     ...
     * these instances.
     */
    public TokenChoice tokenChoice (Parser... baseParsers)
    {
        Parser[] parsers1 = NArrays.map(baseParsers, new Parser[0], it ->
        {
     ...
     */
    boolean parseToken (Parse parse, Parser target)
    {
        Memoizer memo = memoState.data(parse);
        MemoEntry e = memo.get(null, parse.pos, null);

        if (e == null) // token for position not in table yet
            e = fillCache(memo, parse);
     ...
     * <p>In all cases, fills the cache with the tokenization result for the current position.
     */
    boolean parseTokenChoice (Parse parse, Parser[] targets)
    {
        Memoizer memo = memoState.data(parse);
        MemoEntry e = memo.get(null, parse.pos, null);
     ...
     */
    private MemoEntry fillCache (Memoizer memo, Parse parse)
    {
        int pos0 = parse.pos;
        ...
        MemoEntry entry = new MemoEntry(
            success, success ? parsers[longest] : null, pos0, maxPos, delta, null);
